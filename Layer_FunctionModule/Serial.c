...
void SerialPackTransmit(void* pData)
{
	...
	memcpy(Pack.Data, pData, SERIAL_DATABYTE);
...
{
	// 帧起始校验
	if(pPack->FrameStart == FRAMESTART)
	{
		// 帧数据长度校验
		if(pPack->DataByte == SERIAL_DATABYTE)
		{
			// 帧数据接收
			memcpy(pData, pPack->Data, SERIAL_DATABYTE);
			// 帧结尾校验
			if(pPack->FrameEnd == FRAMEEND)
				return;
		}
	}
	AlarmLED();
